...
from wechatUtils import WX_MSG_TYPE_MAP,KEY_QUEUE_WXMSG_SEND,KEY_QUEUE_WXMSG_RECV,wx_msg_tostring_full
import utils
import time
...
def __msg_recv_inqueue(msg):
    try:
        ...
        my_wx_id = my_info['wx_id']
        msg_queue.put(msg)
        log.info(f'wechat-recv-msg-inqueue [{wx_acc}/{my_wx_id}]-> {wx_msg_tostring_full(msg)}')
...
def start(msg_handler=None):
    ...
    wx = WeChatPYApi(msg_callback=__on_msg_callback, exit_callback=__on_exit_callback, logger=log)
    ...
    while True:
        ...
        if msg_recv:
            ...
            if type == 100:
                if msg_handler:
                    try:
                        msg_handler(wx,my_info,msg_recv)
                    except Exception as e:
                        log.error(e,exc_info=True)
                else:
